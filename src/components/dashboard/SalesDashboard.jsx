      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          {mockData.applications.map((a, i) => (
            <div key={i} className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 last:border-0 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              <div className="flex-grow min-w-0">
                <p className="text-sm font-black italic text-gray-800 dark:text-gray-100">{a.company}</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">{a.type}</p>
              </div>
        ...
        >
          {mockData.employees.map((e, i) => (
            <div key={i} className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 last:border-0 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              <div className="flex-grow min-w-0">
                <p className="text-sm font-black italic text-gray-800 dark:text-gray-100">{e.company} — {e.count}</p>
        ...
        <Block title="Reservations expiring soon">
          {mockData.reservations.map((r, i) => (
            <div key={i} className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 last:border-0 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              <div className="flex-grow min-w-0">
                <p className="text-sm font-black italic text-gray-800 dark:text-gray-100">{r.company} — {r.rooms}</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">{r.city}</p>
              </div>
