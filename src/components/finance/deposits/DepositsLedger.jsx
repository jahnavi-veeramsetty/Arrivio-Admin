import { Search, Shield, Download, AlertCircle, Trash2, ArrowRight } from 'lucide-react';

export default function DepositsLedger({ deposits, onViewDetail }) {
  const totalLiability = deposits.reduce((acc, curr) => acc + curr.amount, 0);
              ...
              {deposits.map((d, i) => (
                ...
                >
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{d.tenant}</p>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">{d.id}</p>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <p className="text-sm font-black text-gray-900 dark:text-gray-100">£{d.amount.toLocaleString()}</p>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <p className={`text-sm font-black ${d.deductions > 0 ? 'text-red-500' : 'text-gray-300'}`}>
                      -£{d.deductions.toLocaleString()}
                    </p>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-[11px] font-bold text-gray-500">
                      {new Date(d.moveOut).toLocaleDateString()}
                    </p>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                      d.status === 'Held' ? 'bg-blue-50 text-blue-700 border-blue-100' : 
                      d.status === 'Returning' ? 'bg-amber-50 text-amber-700 border-amber-100 italic' : 
