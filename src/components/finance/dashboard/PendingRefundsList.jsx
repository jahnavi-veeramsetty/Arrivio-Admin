          ...
          <div key={index} className="p-4 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">{refund.reason}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-black text-amber-600">€{refund.amount.toLocaleString()}</p>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tight">{new Date(refund.date).toLocaleDateString()}</p>
              </div>
            </div>

            <div className="flex items-center justify-between gap-3 mt-4">
              <div className="flex items-center gap-1.5 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                <User size={10} className="text-gray-300" /> {refund.admin.split(' ')[0]}
              </div>
