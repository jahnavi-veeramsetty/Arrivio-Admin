    ...
    <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-6 border-b border-gray-50 dark:border-gray-700 flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-gray-400 font-bold mt-1 uppercase">Unified feed (B2C, B2B, and services)</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
            {transactions.map((transaction, index) => {
              ...
              return (
                <tr key={index} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors group">
                  <td className="px-6 py-4">
                    <span className="text-[11px] font-mono font-bold text-gray-400 group-hover:text-[#1a6644] transition-colors">{transaction.id}</span>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-[13px] font-bold text-gray-800 dark:text-gray-200">{transaction.user}</p>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-gray-500 uppercase tracking-tight">{transaction.type}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                      transaction.status === 'Success' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-amber-50 text-amber-700 border-amber-100'
                    }`}>
                      {transaction.status}
                    </span>
                  </td>
