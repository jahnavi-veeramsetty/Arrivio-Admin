...
export default function PaymentDetail({ payment, isOpen, onClose, onRefund }) {
  ...
  const statusStyles = payment.status === 'Succeeded'
    ? 'bg-green-50/50 border-green-100'
    : payment.status === 'Pending'
      ? 'bg-amber-50/50 border-amber-100'
      : 'bg-red-50/50 border-red-100';

  const statusTextStyles = payment.status === 'Succeeded'
    ? 'text-green-800'
    : payment.status === 'Pending'
      ? 'text-amber-800'
      : 'text-red-800';
    ...
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl animate-in slide-in-from-right duration-500 overflow-y-auto">
        ...
        <div className="p-6 space-y-8">
          <div className={`p-6 rounded-3xl border ${statusStyles} flex items-center justify-between`}>
            <div>
              ...
              <div className="flex items-center gap-2">
                {payment.status === 'Succeeded' ? (
                  ...
                  <AlertTriangle className={payment.status === 'Pending' ? 'text-amber-600' : 'text-red-600'} size={24} />
                )}
                <span className={`text-2xl font-black italic ${statusTextStyles}`}>
                  {payment.status}
                </span>
              </div>
            </div>
          ...
          <section className="bg-gray-50 dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed mb-4">
              This payment was successfully matched with Arrivio&apos;s payment verification controls.
              Funds were settled to the Arrivio operating account on {new Date(payment.date).toLocaleDateString()}.
            </p>
