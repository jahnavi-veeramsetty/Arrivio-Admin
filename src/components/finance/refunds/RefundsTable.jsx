import { Search, Undo2, MoreVertical, Filter, ArrowUpRight } from 'lucide-react';
            ...
            {refunds.map((r, i) => (
              ...
              >
                <td className="px-6 py-4">
                  <span className="text-[11px] font-mono font-bold text-gray-400 group-hover:text-amber-600 transition-colors">{r.id}</span>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{r.tenant}</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-[11px] font-bold text-gray-500 italic">"{r.reason}"</p>
                </td>
                <td className="px-6 py-4 text-right">
                  <span className="text-sm font-black text-amber-600">£{r.amount.toLocaleString()}</span>
                </td>
                <td className="px-6 py-4">
                  <p className="text-[11px] font-bold text-gray-500 uppercase tracking-tighter">{r.admin}</p>
                </td>
                <td className="px-6 py-4 text-center">
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                    r.status === 'Succeeded' ? 'bg-green-50 text-green-700 border-green-100' :
