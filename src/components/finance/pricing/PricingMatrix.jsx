...
export default function PricingMatrix({ pricing, onUpdate }) {
  const [editingCell, setEditingCell] = useState(null); // { city, type }
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white dark:bg-gray-900/50">
              ...
              <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">1-BR Apt</th>
              <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">2-BR Apt</th>
            ...
            {pricing.map((p, i) => (
              <tr key={i} className="hover:bg-gray-50/50 transition-colors group">
                <td className="px-8 py-6">
                  <p className="text-base font-black text-gray-800 dark:text-gray-100 italic tracking-tight">{p.city}</p>
                </td>
                <td className="px-8 py-6">
                   <div className="flex items-center gap-2">
                      <span className="text-sm font-black font-mono">£{p.studio.toLocaleString()}</span>
                ...
                <td className="px-8 py-6">
                   <div className="flex items-center gap-2">
                      <span className="text-sm font-black font-mono">£{p.oneBed.toLocaleString()}</span>
                      <button className="p-1.5 hover:bg-white rounded-lg text-gray-300 hover:text-[#1a6644] transition-all opacity-0 group-hover:opacity-100 border border-transparent hover:border-gray-100">
                         <Edit3 size={12} />
                      </button>
                   </div>
                ...
                <td className="px-8 py-6">
                   <div className="flex items-center gap-2">
                      <span className="text-sm font-black font-mono">£{p.twoBed.toLocaleString()}</span>
                      <button className="p-1.5 hover:bg-white rounded-lg text-gray-300 hover:text-[#1a6644] transition-all opacity-0 group-hover:opacity-100 border border-transparent hover:border-gray-100">
                         <Edit3 size={12} />
                      </button>
                   </div>
                ...
                <td className="px-8 py-6">
                   <div className="flex items-center gap-3">
                      <div className="flex flex-col">
                         ...
                         <span className="text-[10px] font-black text-amber-600">x{p.tiers.short}</span>
                      </div>
                      <div className="w-px h-6 bg-gray-100" />
                      <div className="flex flex-col">
                         <span className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">3-6M</span>
                         <span className="text-[10px] font-black text-blue-600">x{p.tiers.med}</span>
                      </div>
                      <div className="w-px h-6 bg-gray-100" />
                      <div className="flex flex-col">
                         <span className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">6M+</span>
                         <span className="text-[10px] font-black text-green-600">x{p.tiers.long}</span>
                      </div>
                   </div>
                </td>
              </tr>
