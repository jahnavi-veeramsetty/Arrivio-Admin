      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 shadow-2xl animate-in slide-in-from-right duration-500 overflow-y-auto">
        <div className="sticky top-0 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 p-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-4 rounded-2xl ${invoice.status === 'Paid' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
              <FileX2 size={24} />
            </div>
            <div>
        ...
        <div className="p-8 space-y-10">
          ...
          <section>
            ...
            <div className="space-y-4 ml-2 border-l-2 border-gray-50 dark:border-gray-800 pl-6">
              <div className="relative">
                <div className="absolute -left-[31px] top-1 w-3 h-3 rounded-full bg-green-500 border-2 border-white dark:border-gray-900 shadow-sm" />
                <p className="text-xs font-bold text-gray-800 dark:text-gray-200">Invoice Generated</p>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">System Automated · {invoice.sentDate ? new Date(invoice.sentDate).toLocaleDateString() : new Date(invoice.dueDate).toLocaleDateString()}</p>
              </div>
              {invoice.sentDate && (
                <div className="relative">
                  <div className="absolute -left-[31px] top-1 w-3 h-3 rounded-full bg-blue-500 border-2 border-white dark:border-gray-900 shadow-sm" />
              ...
              {invoice.status === 'Paid' && (
                <div className="relative">
                  <div className="absolute -left-[31px] top-1 w-3 h-3 rounded-full bg-[#1a6644] border-2 border-white dark:border-gray-900 shadow-sm" />
                  <p className="text-xs font-bold text-gray-800 dark:text-gray-200">Payment Confirmed</p>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Bank Transfer Matched</p>
                </div>
              )}
            </div>
          </section>
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-950 rounded-2xl border border-gray-100 dark:border-gray-900 flex gap-3 italic">
            <p className="text-[10px] text-gray-500 font-bold leading-relaxed">
              <strong>Financial Note:</strong> Overdue statuses are escalated to partner finance follow-up and tracked in the working capital review.
            </p>
          </div>
        </div>
      </div>
